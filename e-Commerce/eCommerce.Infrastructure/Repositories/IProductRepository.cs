...
public class ProductRepository : GenericRepository<Product>, IProductRepository
{
    ...
    {
        return await _context.Products.Include(p => p.Category).FirstOrDefaultAsync(p => p.Id == id); //_dbSet.FindAsync(id);
    }
    ...
    {
        return await _context.Products.Include(p => p.Category).Where(p => p.Price >= minPrice && p.Price <= maxPrice).ToListAsync();
    }
}